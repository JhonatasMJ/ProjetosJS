const quotes = [
  ...
  const btnFrase = document.querySelector('#gerarCitacao')
  const texto = document.querySelector('.texto')
  const autor = document.querySelector('.autor')


  function pegarCitacao() {
    const aleatorio = Math.floor(Math.random() * quotes.length)  //Pegando a quantidade máxima de elementos dentro do array de forma aleatória e arradondada
    texto.textContent = `"${quotes[aleatorio].quote}"` //Pega o texto da citacao
    autor.textContent = quotes[aleatorio].author //Pega o autor da citação
  }

  btnFrase.addEventListener('click', pegarCitacao)